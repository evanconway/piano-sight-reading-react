...
class Pitch {
    ...
    midi: number;
    string: string;
    defaultAccidental: string;
    constructor(key: string, staffIndex: number, harmony: string | null = null) {
        this.key = key;
...
const getPitchStringFromIndex = function (index: number) {
    let register = 4;
    if (index >= 0) {
        while (index >= 7) {
            index -= 7;
...
const getChordOptions = function (key = "C", min = -15, max = 15, harmony: string | null = null) {
...
class Chord { 
    ...
    constructor(key = "C", indMin = 0, indMax = 15, numOfPitches = 1, duration = 12, harmony: string | null = null) {
        ...
        for (let i = 0; i < numOfPitches && options.length > 0; i++) {
            ...
            let remove = this.staffIndexLowest ? this.staffIndexLowest : 0 + 7;
            let index = 0;
            ...
            remove = this.staffIndexHighest ? this.staffIndexHighest : 0 - 7;
