...
const makeScaleMajor = function(rootLetter: PitchClass, rootMidi: number, rootStaff: number, accidental_arr = SHARPS, accidental_num = 0) {
    const SCALE = new Map();
...
const makeScaleMinor = function (rootLetter: PitchClass, rootMidi: number, rootStaff: number, accidental_arr = SHARPS, accidental_num = 0) {
    const SCALE = new Map();
    let indexL = LETTERS.indexOf(rootLetter);
    SCALE.set(1, [LETTERS[indexL++], rootMidi]);
    SCALE.set(2, [LETTERS[lettersWrap(indexL++)], midiWrap(rootMidi + 2)]);
...
// HARMONY
// Chord progression rules are used for random note generation.
